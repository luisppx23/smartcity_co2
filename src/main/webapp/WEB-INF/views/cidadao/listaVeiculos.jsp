...
<div class="form-page-wrapper">
    ...
    <div class="form-card history-card vehicle-list-card">
        <div class="form-card-header">
            <div class="form-card-logo"> <span>🚘</span> </div>
            <h1 class="form-card-title">Portal Smart City</h1>
            <h2 class="form-card-subtitle">Lista de Veículos</h2>
            <p class="form-card-description"> Veículos associados ao cidadão <strong>${user.username}</strong>. </p>
        </div> <c:if test="${empty cidadao.listaDeVeiculos}">
        <div class="empty-state-box"> Ainda não existem veículos associados à sua conta. </div>
    </c:if>
        <c:if test="${not empty cidadao.listaDeVeiculos}">
            <div class="history-table-wrapper">
                <table class="history-table">
                    ...
                    <tbody>
                    <c:forEach var="ownership" items="${cidadao.listaDeVeiculos}"> <tr>
                        <td>${ownership.matricula}</td>
                        <td>${ownership.veiculo.marca}</td>
                        <!-- Acessa através do veiculo associado -->
